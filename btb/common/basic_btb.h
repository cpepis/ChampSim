...
public:
  void initialize()
  {
    ...
    for (uint32_t i = 0; i < SETS; i++) {
      for (uint32_t j = 0; j < WAYS; j++) {
        ...
        basic_btb[i][j].type = branch_info::ALWAYS_TAKEN;
        basic_btb[i][j].lru = 0;
      }
    }
    lru_counter = 0;
  ...
  std::pair<uint64_t, branch_info> predict(uint64_t ip)
  {
    auto btb_entry = find_entry(ip);
    if (btb_entry == NULL) {
      // no prediction for this IP
      return std::make_pair(0, branch_info::ALWAYS_TAKEN);
    }
    update_lru(btb_entry);
    return std::make_pair(btb_entry->target, btb_entry->type);
  }
  ...
  void update(uint64_t ip, uint64_t branch_target, uint8_t taken, uint8_t branch_type)
  {
    ...
    if (btb_entry == NULL) {
      ...
      btb_entry->ip_tag = ip;
      btb_entry->type = branch_info::ALWAYS_TAKEN;
      update_lru(btb_entry);
    }
    // update btb entry
    if (branch_target != 0)
    ...
    if ((branch_type == BRANCH_INDIRECT) || (branch_type == BRANCH_INDIRECT_CALL)) {
      btb_entry->type = branch_info::INDIRECT;
    } else if (branch_type == BRANCH_RETURN) {
      btb_entry->type = branch_info::RETURN;
    } else if (branch_type == BRANCH_CONDITIONAL) {
      btb_entry->type = branch_info::CONDITIONAL;
    }
  }
};
